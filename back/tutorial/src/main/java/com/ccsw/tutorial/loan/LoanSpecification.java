...
public class LoanSpecification implements Specification<Loan> {
    ...
    public Predicate toPredicate(Root<Loan> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
        if (criteria.getOperation().equalsIgnoreCase(":") && criteria.getValue() != null) {
            ...
            Predicate inicioMenoroIgual = builder.lessThanOrEqualTo(root.get("startDate"), date);
            Predicate finMayoroIgual = builder.greaterThanOrEqualTo(root.get("endDate"), date);
            return builder.and(inicioMenoroIgual, finMayoroIgual);
        }
        return null;
    }
